...
def _clean_omp_02(
    ...
    # Create a column to store plain text component data.
    df[config.COL_ITEM_NAME] = df["primary_component_text"].fillna(
        df["secondary_component_text"]
    )
    ...
    df = assign_unique_record_numbers(
        df, data_type_tag="omp02", file_tag=file_tag, pid=pid
    )
...
def clean_omp_02():
    """Cleans OMP-02 tables.
    See the helper function '_clean_omp_02' for the processing details.
    """
