...
def _clean_ppr_01(
    ...
    """
    This is a helper function that cleans PPR-01 tables.
        Cleaning steps:
            - Convert timedelta values back to timestamps
            - Rename a column that stores diagnosis names.
    ...
    # Rename column
    df = df.rename(columns={"primary_diagnosis_text": config.COL_ITEM_NAME})

    # Drop unnecessary columns and reorder columns
    df = df[final_cols]

    # Remove dots ('.') from ICD-10 codes
    df[config.COL_ITEM_CODE] = df[config.COL_ITEM_CODE].str.replace(
        ".", "", regex=False
    )

    # Assign unique record IDs
    df = assign_unique_record_numbers(
        df, data_type_tag="ppr01", file_tag=file_tag, pid=pid
    )
...
def clean_ppr_01():
    """Cleans PPR-01 tables.
    See the helper function '_clean_ppr_01' for the processing details.
    """
