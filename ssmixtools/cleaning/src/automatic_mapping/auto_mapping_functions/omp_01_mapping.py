"""Module to map codes in OMP-01 records"""
...
def _map_omp_01(
    ...
    This process also inspects irregular codes, such as codes that were not mappable to ATC codes, and its results are recorded in a dictionary.
    ...
    Returns:
        ...
        process_analytics (dict): Results of code mapping inspection.
    """
    # Initialize
    process_analytics = {}
    original_text_cols = ["primary_prescription_text", "secondary_prescription_text"]
    cropped_text_cols = [
        "cropped_primary_prescription_text",
        "cropped_secondary_prescription_text",
    ]
    ...
    df = map_by_merge(
        df,
        hot7_to_yj7,
        original_code_col="primary_prescription_code",
        original_cropped_length=7,
        original_code_pattern=config.R_HOT_CODE,
        original_system_col="primary_prescription_coding_system",
        original_system_pattern=config.R_HOT,
        new_col_name="mapped_yj7",
    )
    # Fill missing 'YJ7' column values with 'secondary prescription code'.
    df["mapped_yj7"] = backup_fill_from_secondary_code(
        df,
        target_col="mapped_yj7",
        secondary_code_col="secondary_prescription_code",
        secondary_coding_system_col="secondary_prescription_coding_system",
    ...
    # Create a new column for ATC by mapping YJ7 to ATC
    df = map_by_merge(
        df,
        yj7_to_atc,
    ...
    # Filling missing 'ATC' column values with 'backup ATC'.
    ...
    hot_existing_mask = df["primary_prescription_coding_system"].str.contains(
        config.R_HOT, na=False
    ) & ~(df["primary_prescription_code"].isna())
    ...
    unmappable_hot_counts = all_values_to_int(
        df[unmappable_hot_mask]["primary_prescription_code"].value_counts().to_dict()
    )

    all_unmappables = all_values_to_int(
        df[missing_atc_mask | irregular_atc_mask]["primary_prescription_text"]
        .value_counts()
        .to_dict()
    )
...
def map_omp_01():
    """Maps medication prescription records in OMP-01 tables to ATC codes.

    See the helper function '_map_omp_01' for the processing details.
